...
```{r}
# Generate 100 random numbers from a Poisson distribution with lambda = 3
random_numbers <- rpois(100, lambda = 3)
...
quantile_value <- qpois(0.95, lambda = 3)
...
cont_table <- table(df$region, df$status)
...
```
```{r}
power_analysis <- pwr.chisq.test(w=ES.w1(expected_frequency, observed_frequency), df=(12-1), power=0.8, sig.level = 0.05)
power_analysis
```
